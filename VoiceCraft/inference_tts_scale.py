...
@torch.no_grad()
def inference_one_sample_graphemes(model, model_args, phn2num, audio_tokenizer, audio_fn, target_text, device, decode_config, prompt_end_frame):
    # phonemize

    def tok_text(text):
        phn = text.split(" ")
        phn = list(map(lambda x: " ".join([y for y in x]), phn))
        phn = "_".join(phn)
        phn_seq = " ".join(phn)
        return phn_seq

    text_tokens = [phn2num[phn] for phn in
            tok_text(target_text.strip()) if phn in phn2num
        ]
    ...
    # # print('enc_frames', len(encoded_frames), encoded_frames[0])
    # print('ec', len(encoded_frames[0]))
    original_audio = encoded_frames[0][0].transpose(2,1) # [1,T,K]
    print('og', original_audio.shape)
    ...
    if decode_config['sample_batch_size'] <= 1:
        ...
        print('size', text_tokens.shape, original_audio[...,:model_args.n_codebooks].shape)
        ...
        print('multi size', text_tokens.shape, original_audio[...,:model_args.n_codebooks].shape)
