...
class Trainer:
        ...
        if self.rank == 0:
            ...
            if args.use_wandb:
                if not args.wandb_project:
                    raise ValueError("wandb_project must be provided")
        ...
        while flag:
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                if self.rank == 0 and self.progress['step'] % self.args.tb_write_every_n_steps == 0:
                    ...
                    wandb.log({"train/lr": cur_lr}, step=self.progress['step'])
                ...
                for j in range(self.args.gradient_accumulation_steps):
                    ...
                    if self.rank == 0:
                        ...
                        if self.progress['step'] % self.args.tb_write_every_n_steps == 0:
                            ...
                            wandb.log({"train/loss": average_loss}, step=self.progress['step'])
                            self.writer.add_scalar('train/top10acc', average_top10acc, self.progress['step'])
                            wandb.log({"train/top10acc": average_top10acc}, step=self.progress['step'])
                            self.writer.add_scalar("train/ntokens", sum_ntoken, self.progress['step'])
                            wandb.log({"train/ntokens": sum_ntoken}, step=self.progress['step'])
                            for cb in range(self.args.n_codebooks):
                                self.writer.add_scalar(f'train/top10acc_cb{cb+1}', average_top10acc_cbi[cb], self.progress['step'])
                                wandb.log({f'train/top10acc_cb{cb+1}': average_top10acc_cbi[cb]}, step=self.progress['step'])
                ...
                if self.rank == 0:
                    logging.info(f"Epoch {self.progress['epoch']},\t Step {self.progress['step']} / {self.total_step}")
                    ...
                    if self.progress['step'] % self.args.tb_write_every_n_steps == 0:
                        ...
                        wandb.log({"train/data_time": data_end_time - data_start_time}, step=self.progress['step'])
                        self.writer.add_scalar("train/train_time", time.time() - data_end_time, self.progress['step'])
                        wandb.log({"train/train_time": time.time() - data_end_time}, step=self.progress['step'])
                    ...
                    if self.progress['step'] % self.args.print_every_n_steps == 0:
                        ...
                        wandb.log({"train/cur_epoch": log_out['cur_epoch']}, step=self.progress['step'])
                        log_out['cur_step'] = f"{int(self.progress['cur_step']+1)}"
                        log_out['total_step'] = f"{self.progress['step']}/{self.args.num_steps}"
                        wandb.log({"train/total_step": log_out['total_step']}, step=self.progress['step'])
    ...
    def validate(self, valid_loader=None, hide_progress=True):
        ...
        if self.rank == 0:
            ...
            wandb.log({"val/loss": val_loss}, step=self.progress['step'])
            ...
            wandb.log({"val/top10acc": val_top10acc}, step=self.progress['step'])
            for cb in range(self.args.n_codebooks):
                ...
                wandb.log({f'val/top10acc_cb{cb+1}': average_top10acc_cbi}, step=self.progress['step'])
    ...
    def _setup_models(self):
        ...
        if self.args.load_model_from is not None and self.progress['step'] <= 1:
            ...
            old_token_lim = sd["text_embedding.word_embeddings.weight"].size(0) - 1
            old_embeddings = sd["text_embedding.word_embeddings.weight"][:old_token_lim-1,]
            mean = torch.mean(old_embeddings, dim=0)
            std = torch.std(old_embeddings, dim=0)
            new_embeddings = torch.from_numpy(
                np.random.normal(
                    loc=mean.numpy(), 
                    scale=std.numpy(), 
                    size=(self.args.text_vocab_size - old_token_lim, self.args.d_model)
                )
            )

            sd["text_embedding.word_embeddings.weight"] = torch.cat(
                    (
                        sd["text_embedding.word_embeddings.weight"][:old_token_lim,], 
                        new_embeddings, 
                        sd["text_embedding.word_embeddings.weight"][old_token_lim:,]
                    )
                )
