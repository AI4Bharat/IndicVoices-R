...
class VoiceCraft(
    ...
    def get_starts_ends_original(self, y_len):
    ...
    def get_starts_ends_with_prompt(self, y_len, y1_len):
        n_spans = 1
        starts = [y1_len]
        ends = [y_len-1]
        return starts, ends   
    ...
    def prepare_mask_intervals_with_prompt(self, y_lens, y1_lens):
        mask_intervals = []
        non_mask_intervals = []

        for i, (y_len, y1_len) in enumerate(zip(y_lens, y1_lens)):
            if y1_len == y_len: # no additional prompt, original training setup
                starts, ends = self.get_starts_ends_original(y_len)
            else: # with additional prompt
                starts, ends = self.get_starts_ends_with_prompt(y_len, y1_len)
            # print(starts, ends, y_len, y1_len)
            # starts, ends = self.get_starts_ends_original(y_len)
    ...
    def prepare_input_target(self, y, y_lens, y1_lens):
        # rearrange y
        # assume y shape: [B T K], K is n_codebooks
        # assert y.shape[1] == self.args.n_codebooks, y.shape
        # sample mask_intervals
        mask_intervals, non_mask_intervals = self.prepare_mask_intervals_with_prompt(y_lens, y1_lens)
    ...
    def dec_forward(
            ...
            # print("LN 419 ", x_input.shape, y_input.shape)
            x_attn_mask = F.pad(
                x_attention_mask,
                (0, new_y_lens.max()),
                value=True,
            ...
            if past == None: # do not use kvcache
                ...
                # print("LN 465", out[0], x_lens.max(), out[0].shape)
    ...
    def forward(self, batch):
        ...
        Args:
          ...
          y_lens:
            ...
            before padding.
        """
        x, x_lens, y, y_lens, y1_lens = batch["x"], batch["x_lens"], batch["y"], batch["y_lens"], batch["y1_lens"]
        if len(x) == 0:
            return None
        ...
        y_input, new_y_lens, targets, y_padding_mask, y_attention_mask, mask_position, patterns = self.prepare_input_target(y, y_lens, y1_lens)
